...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupTabRecycler() {
        tabsRecycler.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));
    ...
    private void configureWebView(WebView web) {
        ...
        web.setDownloadListener((url, ua, cd, mime, length) -> {
            try {
                ...
                if (dm != null) { dm.enqueue(req); Toast.makeText(this, "İndirme başladı: " + fileName, Toast.LENGTH_SHORT).show(); }
            } catch (Exception e) { Toast.makeText(this, "İndirme hatası", Toast.LENGTH_SHORT).show(); }
        });
    }
    ...
    private void setupClickListeners() {
        ...
        urlInput.setOnEditorActionListener((v, id, e) -> { navigate(); hideKeyboard(); return true; });
        ...
        menuHistory.setOnClickListener(v -> {
            ...
            BottomSheetPanel.showHistory(this, items, dbHelper, new BottomSheetPanel.OnItemClickListener() {
        ...
        menuBookmarks.setOnClickListener(v -> {
            ...
            if (!isHome) {
                // Sayfadayken: yer imine ekle/göster seçeneği
                boolean alreadySaved = dbHelper.isBookmarked(currentUrl);
                if (!alreadySaved) {
                    boolean added = dbHelper.addBookmark(currentUrl, webView.getTitle());
                    Toast.makeText(this, added ? "⭐ Yer imine eklendi" : "Zaten kayıtlı", Toast.LENGTH_SHORT).show();
                } else {
                    // Zaten ekliyse listeyi göster
                    showBookmarkList();
                }
            } else {
                showBookmarkList();
        ...
        menuAdBlock.setOnClickListener(v -> {
            ...
            Toast.makeText(this, next ? "🛡 Gizlilik Tüneli Aktif" : "Standart Bağlantı", Toast.LENGTH_SHORT).show();
        });

        // AYARLAR = masaüstü modu toggle
    ...
    private void showBookmarkList() {
        ...
        BottomSheetPanel.showBookmarks(this, items, new BottomSheetPanel.OnItemClickListener() {
            @Override public void onItemClick(String url) { webView.loadUrl(url); }
    ...
    private void navigate() {
        ...
        if (!url.startsWith("http://") && !url.startsWith("https://")) {
            url = url.contains(".") && !url.contains(" ") ? "https://" + url
